...
fn print_help() {
    println!("md5sum - Compute and check MD5 checksums");
    println!();
    println!("USAGE:");
    println!("    md5sum [OPTIONS] [FILE...]");
    println!();
    println!("OPTIONS:");
    println!("    -c, --check       Read MD5 sums from FILE and check them");
    println!("    --help            Print this help message");
    println!();
    println!("DESCRIPTION:");
    println!("    With no FILE, or when FILE is -, read standard input.");
    println!("    The sums are computed as described in RFC 1321.");
    println!();
...
fn read_file(path: &str) -> Result<Vec<u8>, String> {
    if path == "-" {
        ...
        io::stdin().read_to_end(&mut buffer)
            .map_err(|e| format!("md5sum: failed to read stdin: {}", e))?;
        Ok(buffer)
    } else {
        fs::read(path)
            .map_err(|e| format!("md5sum: cannot read '{}': {}", path, e))
    }
}
    ...
    for check_file in files {
        let content = if check_file == "-" {
            ...
            fs::read_to_string(check_file)
                .map_err(|e| format!("md5sum: cannot read '{}': {}", check_file, e))?
        };
        ...
        for line in content.lines() {
            ...
            if parts.len() != 2 {
                eprintln!("md5sum: {}: improperly formatted MD5 checksum line", check_file);
                all_ok = false;
                continue;
            }
            ...
            match read_file(filename) {
                Ok(data) => {
                    let actual_hash = compute_md5(&data);
